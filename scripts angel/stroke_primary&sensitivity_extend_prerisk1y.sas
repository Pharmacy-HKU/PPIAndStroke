...
Purpose: replicate the stroke SAS editor apart from extending the pre-risk period to 1y
...
data all2;
set all;
run; *note:146030, after: stop macro until entries in aa is 0;
...
data all3;
set all;
run; *, after: 146030 stop macro until entries in aa is 0;
%macro multi(max=, input=);
%do i=1 %to &max;
proc sort data=&input;
by reference_key decending pst decending pen;run;
...
data all4;
set all2 all3;
run; *note: 292060, 146317 stop macro until entries in aa is 0;
...
proc sort data=primary_risk5;
by reference_key pst pen r;run; *136998;

proc sort data=st.ppi_stroke_coh1_adult nodupkey out=ppi_hc;
by reference_key;
run;
proc sql;
create table st.primary_risk_final_prerisk1y as
select * from primary_risk5 A left join (select sex as gen from ppi_hc B)
on A.reference_key=B.reference_key;
quit; *136998;
data st.primary_risk_final_prerisk1y;
set st.primary_risk_final_prerisk1y;
if gen='M' then sex=1;
if gen='F' then sex=0;
run;
data st.primary_risk_final_prerisk1y;
...
proc sql;
create table st.primary_risk_final_prerisk1y as
select * from st.primary_risk_final_prerisk1y A left join (select dob, ppirxst, eventd from ppi_hc B)
on A.reference_key=B.reference_key;
quit; *136998;
...
select *, max(evt) as max_evt from st.primary_risk_final_prerisk1y group by reference_key;
quit;
data rm;
...
run; *6664;
proc sql;
create table st.primary_risk_final_prerisk1y2 as
select * from st.primary_risk_final_prerisk1y where reference_key not in (Select reference_key from rm);
quit; *125241;
proc sort data=st.primary_risk_final_prerisk1y2 nodupkey out=final_hc;
by reference_key;
run; *8491, two more were captured because the longer the pre-risk period so captured those were excluded due to 
...
*key censored reason;
proc sql;
create table primary_risk_final_prerisk1y2 as
select * from st.primary_risk_final_prerisk1y2 A left join (Select censor_reason from ppi_stroke_coh1_adult_censor B)
on A.reference_key=B.reference_key;
quit;

/*making R dataset*/
data poisson_add;
set primary_risk_final_prerisk1y2;
indiv=reference_key;
gender=sex;
...
PROC EXPORT DATA= poisson_add
            OUTFILE= "I:\PPIs & stroke\R dataset\sccs_ppistroke_allrx_prerisk1y.txt" 
            DBMS=TAB REPLACE;
     PUTNAMES=NO;
RUN;
...
data all2;
set all;
run; *note: 133395, after: stop macro until entries in aa is 0;
...
data all4;
set all2 all3;
run; *note:266790, after: stop macro until entries in aa is 0;
...
proc sort data=primary_risk4;
by reference_key pst pen r;run; *133429;


proc sql;
create table st.sen1_primary_final_prerisk1y as
select * from primary_risk4 A left join (select sex as gen from ppi_hc B)
on A.reference_key=B.reference_key;
quit; *133429;
data st.sen1_primary_final_prerisk1y;
set st.sen1_primary_final_prerisk1y;
if gen='M' then sex=1;
if gen='F' then sex=0;
run;
data st.sen1_primary_final_prerisk1y;
...
proc sql;
create table st.sen1_primary_final_prerisk1y as
select * from st.sen1_primary_final_prerisk1y A left join (select dob, ppirxst, eventd from ppi_hc B)
on A.reference_key=B.reference_key;
quit; *133429;
...
*key censored reason;
proc sql;
create table sen1_primary_final_prerisk1y as
select * from st.sen1_primary_final_prerisk1y A left join (Select censor_reason from ppi_stroke_coh1_adult_censor B)
on A.reference_key=B.reference_key;
quit;
data poisson_sen1;
set sen1_primary_final_prerisk1y;
indiv=reference_key;
gender=sex;
astart=datdif(dob, st_st, 'act/act');
adrug=datdif(dob, ppirxst, 'act/act');
...
PROC EXPORT DATA= poisson_sen1
            OUTFILE= "I:\PPIs & stroke\R dataset\sen1_primary_risk_final_prerisk1y.txt" 
            DBMS=DLM REPLACE;
     PUTNAMES=NO;
RUN;

proc sort data=st.sen1_primary_final_prerisk1y nodupkey out=sen1_hc;
by reference_key;run; *8974;
