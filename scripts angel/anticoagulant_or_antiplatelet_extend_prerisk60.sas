...
but using the pre-risk period of 60 days*/
...
data all2;
set all;
run; *note:171549, after: stop macro until entries in aa is 0;
...
data all4;
set all2 all3;
run; *note:343098  stop macro until entries in aa is 0;
...
by reference_key pst pen;
run; *162440;
data st.all4_aa_primary_prerisk60;
set all4_aa;
...
select * from st.all4_aa_primary_prerisk60 A left join (select pst as pre_pst, pen as pre_pen from prerisk B)
on A.reference_key=B.reference_key;
quit; *;
data all4_aa_rw;
set all4_aa_rw;
...
proc sql;
create table st.all4_aa_rw_primary_prerisk60 as
select * from all4_aa_rw5 A left join (select sex as gen from ppi_hc B)
on A.reference_key=B.reference_key;
quit; *152542;
data st.all4_aa_rw_primary_prerisk60;
set st.all4_aa_rw_primary_prerisk60;
if gen='M' then sex=1;
if gen='F' then sex=0;
run;
data st.all4_aa_rw_primary_prerisk60;
...
proc sql;
create table st.all4_aa_rw_primary_prerisk60 as
select * from st.all4_aa_rw_primary_prerisk60 A left join (select dob, ppirxst, eventd from ppi_hc B)
on A.reference_key=B.reference_key;
quit; *152763;
...
select *, max(evt) as max_evt from st.all4_aa_rw_primary_prerisk60 group by reference_key;
quit;
data rm;
...
run; *7102;
proc sql;
create table st.all4_aa_rw_primary_prerisk602 as
select * from st.all4_aa_rw_primary_prerisk60 where reference_key not in (Select reference_key from rm);
quit; *144793;
proc sort data=st.all4_aa_rw_primary_prerisk602 nodupkey out=final_hc;
by reference_key;
run; *8487;

*key censored reason;
proc sql;
create table all4_aa_rw_primary_prerisk602 as
select * from st.all4_aa_rw_primary_prerisk602 A left join (Select censor_reason from ppi_stroke_coh1_adult_censor B)
on A.reference_key=B.reference_key;
quit;
/*making R dataset*/
data poisson_aa;
set all4_aa_rw_primary_prerisk602;
indiv=reference_key;
gender=sex;
...
PROC EXPORT DATA= poisson_aa
            OUTFILE= "I:\PPIs & stroke\R dataset\sccs_ppistroke_main_anticoag_prerisk60.txt" 
            DBMS=TAB REPLACE;
     PUTNAMES=NO;
RUN;
...
data all2;
set all;
run; *note:158805, after:158951 stop macro until entries in aa is 0;
...
data all4;
set all2 all3;
run; *note:317610, after: stop macro until entries in aa is 0;
...
select * from st.all4_aa_sen_prerisk60 A left join (select pst as pre_pst, pen as pre_pen from prerisk B)
on A.reference_key=B.reference_key;
quit; *;
data primary_risk;
set primary_risk;
...
proc sql;
create table st.sen2_primary_final_prerisk60 as
select * from primary_risk4 A left join (select sex as gen from ppi_hc B)
on A.reference_key=B.reference_key;
quit; *149715;
data st.sen2_primary_final_prerisk60;
set st.sen2_primary_final_prerisk60;
if gen='M' then sex=1;
if gen='F' then sex=0;
run;
data st.sen2_primary_final_prerisk60;
...
proc sql;
create table st.sen2_primary_final_prerisk60 as
select * from st.sen2_primary_final_prerisk60 A left join (select dob, ppirxst, eventd from ppi_hc B)
on A.reference_key=B.reference_key;
quit; *149715;
...
proc sort data=st.sen2_primary_final_prerisk60 nodupkey out=sen2_hc;
by reference_key;run;

*key censored reason;
proc sql;
create table sen2_primary_final_prerisk60 as
select * from st.sen2_primary_final_prerisk60 A left join (Select censor_reason from ppi_stroke_coh1_adult_censor B)
on A.reference_key=B.reference_key;
quit;
data poisson_sen2;
set sen2_primary_final_prerisk60;
indiv=reference_key;
gender=sex;
astart=datdif(dob, st_st, 'act/act');
adrug=datdif(dob, ppirxst, 'act/act');
...
PROC EXPORT DATA= poisson_sen2
            OUTFILE= "I:\PPIs & stroke\R dataset\sen2_primary_risk_final_prerisk60.txt" 
            DBMS=DLM REPLACE;
     PUTNAMES=NO;
RUN;
